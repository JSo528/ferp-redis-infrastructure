...
class FerpRedisInfrastructureStack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    const redisConfig = {
      ...
      // ElastiCache only accepts major.minor — patch version (7.0.7) is selected automatically
      engineVersion: '7.0',
    ...
    // Single staging replication group shared by all sports, using the same
    // node type as production so staging behaviour is representative, but only
    // one node (no replicas) to keep staging costs down. Network placement and
    // snapshot/maintenance windows keep the existing ferp-staging settings.
    new elasticache.CfnReplicationGroup(this, 'FerpStagingRedis', {
      replicationGroupDescription: 'Redis replication group for ferp staging (all sports)',
      replicationGroupId: 'ferp-staging',

      ...redisConfig,

      // Staging nodes live in their own subnet group, separate from production
      cacheSubnetGroupName: 'ferp-staging-redis',

      // One node only — automatic failover and Multi-AZ require replicas
      numCacheClusters: 1,
      automaticFailoverEnabled: false,
      multiAzEnabled: false,

      // Existing staging windows, offset from the production ones
